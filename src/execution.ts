...
export interface Executor {
  estimateCompensation(troves: Vault[], price: Decimalish): Decimal;

  execute(
    liquidation: PopulatedBitcoinsMoneypTransaction<LiquidationDetails>
  ): Promise<ExecutionResult>;
}

const addTroves = (troves: Vault[]) => troves.reduce((a, b) => a.add(b), new Vault());
...
const expectedCompensation = (
  ...
  addTroves(troves)
    ...
    .add(BPD_LIQUIDATION_RESERVE.mul(troves.length));
...
class LiqbotExecutor implements Executor {
  private readonly _store: BlockPolledMoneypStore;
...
export const getExecutor = async (store: BlockPolledMoneypStore): Promise<Executor> => {
